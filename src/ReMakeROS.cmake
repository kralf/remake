...
#   ROS_${PACKAGE}_INCLUDE_DIRS, and ROS_${PACKAGE}_LIBRARIES are initialized
#   accordingly. In addition, the directories in which the linker will look for
#   the package libraries is specified by calling CMake's link_directories().
...
macro(remake_ros_find_package ros_package)
  ...
  if(ROS_FOUND)
    ...
    remake_var_name(ros_includes_var ROS ${ros_package} INCLUDE_DIRS)
    ...
    if(ros_result)
      ...
      execute_process(
        COMMAND ${ROSPACK_EXECUTABLE} libs-only-L ${ros_package}
        OUTPUT_VARIABLE ros_link_directories
        OUTPUT_STRIP_TRAILING_WHITESPACE)

      remake_set(${ros_path_var} ${${ros_path_var}} CACHE PATH
        "Path to ROS package ${ros_package}.")
      string(REGEX REPLACE "[ ]+" ";" ${ros_includes_var} ${ros_includes})
      string(REGEX REPLACE "[ ]+" ";" ${ros_libraries_var} ${ros_libraries})
      string(REGEX REPLACE "[ ]+" ";" ros_link_directories
        ${ros_link_directories})

      link_directories(${ros_link_directories})
    endif(ros_result)
